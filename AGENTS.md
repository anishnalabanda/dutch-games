...
interface ProgressStore {
  ...
  all(): Promise<Record<string, any>>;            // every nl.* key, for export
  bulk(data: Record<string, any>): Promise<void>; // for import / merge
}
```
...
try/catch; degrade to in-memory if unavailable. The app is fully usable logged-out and
offline on this layer alone.

### Layer 2 — cloud (Supabase, cross-device sync)
- Free tier: Postgres + auth + a client ("anon"/publishable) key that is SAFE to ship in
  the browser **because Row Level Security is enabled** so a user only ever reads/writes
  their own rows. Put the Supabase **URL** and **anon key** in a config module
  (`src/config.ts`); they are not secrets. NEVER put the service-role key in the client.
- **Auth method: TBD by owner** (magic-link email vs email+password). Implement it behind
  a thin `auth` module so the method can change in one place. Default assumption:
  magic-link email (no password) unless the owner says otherwise.
- **Schema:** table `progress` with columns:
  `user_id uuid` (references `auth.users`), `key text`, `value jsonb`,
  `updated_at timestamptz default now()`, primary key `(user_id, key)`.
  RLS policies: enable RLS; policy `user_id = auth.uid()` for select/insert/update/delete.
- **`SupabaseStore`** implements `ProgressStore` against that table for the signed-in user.
- **Sync strategy:** local is the working copy; cloud is the sync copy.
  ...
  - Logged-out or offline: local only; sync on next sign-in / reconnect.
- Selecting the store lives in ONE provider/context: logged-in -> cloud-backed (with local
  cache); logged-out -> local only. Games are unaware which is active.

### Export / Import (always available, both layers)
- **Export**: button serializing the full `all()` result (every `nl.*` key) into one
  small, human-readable JSON snapshot to copy. This is a backup, an offline cross-device
  bridge, AND how the owner reports progress to Claude in chat (Claude reads the paste).
- **Import**: textarea accepting a pasted snapshot -> `bulk()` (merged, not blind
  overwrite).
- Snapshot shape:
  `{ "version": 1, "exported": "<ISO date>", "data": { "<key>": <value>, ... } }`.
  Keep the format stable across versions.
...
Supabase `progress` table, even while signed in. The `AuthBar` says "Lokaal — geen
cloud-sync" instead of offering a sign-in that would not sync. Set
`VITE_CLOUD_SYNC=on` in a local `.env` to deliberately test sign-in and sync.
Production builds are unaffected.

### Honest caveats to surface in-app where relevant
- Anon key + RLS is the correct, safe pattern for a static SPA; do not treat the anon key
  as a leak.
...
- Cross-device sync requires being signed in on each device.
...
streak, correct count, progress bar), and an `AuthBar` / account menu (sign in/out, sync
